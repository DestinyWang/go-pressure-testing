...
// <param wg>
// <param request>
func DoHttp(ctx context.Context, chanId uint64, ch chan *model.RequestResult, totalNum uint64, wg sync.WaitGroup, req *model.Request) {
	defer wg.Done()
	for n := uint64(0); n < totalNum; n++ {
		...
		if isSucceed, errCode, err = doHttp(req); err != nil {
			return
		}
...
func doHttp(req *model.Request) (isSucceed bool, errCode int, err error) {
